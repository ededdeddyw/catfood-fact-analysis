...
**実測した開示率（2026-06-21 更新・大手詳細ページ含む308成分ページ）**
- 原材料100% / 水分・脂肪・灰分99% / たんぱく質98% / カロリー85% / **リン 38%**
- 大手の保証成分は**静的HTMLで取れる**（詳細URLは各社sitemapに列挙。`harvest_sitemap_products.py`）。Playwright不要だった
- **リン開示はメーカー方針で二極化**：日本PF 93% / アイシア 53% / いなば 4%（ちゅ~る系で非記載・製品数多）
- → 判定は **「縮小版＝リン開示メーカーに絞れば腎臓シート成立」**（母体137ページ）。体重管理ローンチはさらに盤石
- ⚠️ 6/20の「リン8%→見送り」はtail偏重の誤り。経緯と詳細は [docs/04](docs/04_acquisition_audit_results.md) 冒頭の追記

**次の一手（おすすめ順）**
1. **体重管理を第1ローンチに確定**（カロリー85%・原材料100%で盤石）
2. **抽出の精緻化**：保証成分ブロックに限定（アイシアのリン誤検出対策）／カロリー単位 `/個・/本` 区別／全角数字の半角化 → 再ハーベスト
3. **要確認52社のURL確定**（[data/maker_url_todo.csv](data/maker_url_todo.csv) 記入 → `verify_proposed_sites.py` で検証）
4. **療法食メーカー（ヒルズ・ロイヤルカナン）をsitemap対応に追加** → リン開示はさらに上がる見込み（縮小版→GO化）
