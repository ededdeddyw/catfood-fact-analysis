...
**実測した開示率（2026-06-22 精緻化後・544成分ページ／26社）**
- 原材料98% / 脂肪・繊維・水分96% / 灰分95% / たんぱく質91% / **カロリー94%** / **リン 39%**
- 抽出精緻化（保証成分ブロック限定・全角半角・カロリー単位）でカロリーが65%→**94%**に改善、リンは誤検出が減り39%（より正確）
- **リン開示はメーカー方針で二極化**：日本PF 93% / RC 58% / スマック58% / Y.K. 57% / アイシア 53% / いなば 4%
- → **縮小版＝リン開示メーカーに絞れば腎臓シート成立**（母体294ページ）。体重管理はカロリー94%・原材料98%で盤石
- ⚠️ 6/20の「リン8%→見送り」はtail偏重の誤り。経緯と詳細は [docs/04](docs/04_acquisition_audit_results.md) 冒頭の追記

**MVPプロトタイプ（実データ・ブラウザで開ける）**
- [prototype/consult/index.html](prototype/consult/index.html)：猫483商品の比較メモ。乾物量換算・4状態・**ランキング無し**・出典付き・リン開示フィルタ（腎臓ビュー）。設計は [docs/06](docs/06_mvp_design.md)

**次の一手**
1. **抽出のさらなる精緻化**：ラベルと数値の近接誤対応（一部でリンがたんぱく質値を誤取得）を是正
2. **データ0の32社をPlaywright対応** → カバレッジ拡大（[docs/04](docs/04_acquisition_audit_results.md)）
3. **MVPの肉付け**：価格・購入リンク（全商品平等）、母集団カバレッジ表示、印刷/PDF
