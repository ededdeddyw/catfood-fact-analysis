...
**実測した開示率（2026-06-22 最終・確定58社中データ取得26社／557成分ページ）**
- 原材料97% / 脂肪・繊維・水分97% / 灰分94% / たんぱく質92% / カロリー65% / **リン 40%**
- 大手は sitemap（`harvest_sitemap_products.py`）、中小は人手URLからBFS（`harvest_seed_sites.py`）で静的取得。Playwright不要だった
- **リン開示はメーカー方針で二極化**：日本PF 93% / スマック64% / RC 55% / アイシア 53% / いなば 4%（小規模でも100%開示の社あり）
- → 判定 **GO（腎臓シート成立）**：中小多数を足してもリン40%維持・02の撤退ライン≥40%。母体300ページ。体重管理も同時に盤石
- カバレッジ：確定58社中**26社・557ページ取得／残32社は0件**（JS描画等＝Playwright候補）
- ⚠️ 6/20の「リン8%→見送り」はtail偏重の誤り。経緯と詳細は [docs/04](docs/04_acquisition_audit_results.md) 冒頭の追記

**次の一手（おすすめ順）**
1. **腎臓シート＆体重管理を両輪でMVP設計**（リン40%・カロリー/原材料ほぼ全件で両方成立）
2. **抽出の精緻化**：保証成分ブロックに限定（アイシアのリン誤検出対策）／カロリー表記の取りこぼし対応／単位 `/個・/本` 区別／全角数字の半角化／**犬猫分離** → 再ハーベスト
3. **データ0の32社をPlaywright対応**（製品一覧がJS描画）→ カバレッジ拡大（[docs/04](docs/04_acquisition_audit_results.md) に一覧）
4. **要再確認3社・手当て2社**（アイリス/レティシアンのbot対策）の解決
